...
export default function ArtworkGalleryPage() {
  ...
  const fetchGalleryImages = async (uid) => {
    ...
    if (docSnap.exists()) {
      ...
      const fetchedImages = data.artworkGallery || [];
  ...
  const handleFilesUpload = async (e) => {
    ...
    try {
      ...
      for (const file of files) {
        ...
        // Update artworkGallery array in the default doc
        await updateDoc(defaultDocRef, {
          artworkGallery: arrayUnion(downloadURL),
        });
      }
  ...
  const handleDeleteImage = async (imageUrl) => {
    ...
    try {
      ...
      const defaultDocRef = doc(db, 'users', user.uid, 'gallery', 'default');
      await updateDoc(defaultDocRef, {
        artworkGallery: arrayRemove(imageUrl),
      });
  ...
  return (
    <div className="min-h-screen bg-gray-900 text-gray-100 flex flex-col items-center p-8 font-inter">
      <h1 className="text-5xl font-extrabold text-yellow-400 mb-6">Artwork Gallery</h1>
      ...
      {user ? (
        ...
        <p className="text-gray-300">Please sign in to upload and view your artwork.</p>
      )}
    </div>
  );
}