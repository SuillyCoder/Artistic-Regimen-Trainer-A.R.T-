...
export default function GestureModulePage() {
  const [moduleItems, setModuleItems] = useState([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState(null);

  useEffect(() => {
    const fetchGestureModuleItems = async () => {
      try {
        setLoading(true);
        setError(null);
        // Reference to the 'moduleItems' subcollection under 'modules/Gesture'
        const gestureModuleItemsCollectionRef = collection(db, 'modules', 'gesture', 'moduleItems');
        ...
        const q = query(gestureModuleItemsCollectionRef, orderBy('order', 'asc')); 
        ...
        console.error("Error fetching gesture module items:", err);
  ...
  return (
    <div className="min-h-screen bg-gray-900 text-gray-100 flex flex-col items-center p-8 font-inter">
      ...
      <p className="text-lg text-gray-300 mb-10 text-center max-w-2xl">
        Dive deep into the study of human and animal gesture through these curated exercises and resources.
      </p>

      {loading && <p className="text-gray-400 text-xl">Loading gesture exercises...</p>}
      {error && <p className="text-red-500 text-xl">{error}</p>}

      {!loading && !error && moduleItems.length === 0 && (
        <p className="text-gray-400 text-xl">No gesture exercises found yet. Check back later!</p>
      )}
