...
export default function AnatomyModulePage() {
  const [moduleItems, setModuleItems] = useState([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState(null);

  useEffect(() => {
    const fetchAnatomyModuleItems = async () => {
      try {
        setLoading(true);
        setError(null);
        // Reference to the 'moduleItems' subcollection under 'modules/anatomy'
        const anatomyModuleItemsCollectionRef = collection(db, 'modules', 'anatomy', 'moduleItems');
        ...
        const q = query(anatomyModuleItemsCollectionRef, orderBy('order', 'asc')); 
        ...
        console.error("Error fetching Anatomy module items:", err);
  ...
  return (
    <div className="min-h-screen bg-gray-900 text-gray-100 flex flex-col items-center p-8 font-inter">
      ...
      <p className="text-lg text-gray-300 mb-10 text-center max-w-2xl">
        Dive deep into the study of human and animal anatomy through these curated exercises and resources.
      </p>

      {loading && <p className="text-gray-400 text-xl">Loading anatomy exercises...</p>}
      {error && <p className="text-red-500 text-xl">{error}</p>}

      {!loading && !error && moduleItems.length === 0 && (
        <p className="text-gray-400 text-xl">No anatomy exercises found yet. Check back later!</p>
      )}
